...
- 🎯 **策略定型**：确认 v1 原版为最终版本，全仓60%保证金驱动的双周期滚仓
- 📊 **四币全量回测**：ETH +1,122% / BTC +696% / SOL +261% / LAB 淘汰
- 🔬 **版本演进**：v1-v6 六版对比，v1 在所有优化尝试中胜出
- 📋 **风控铁律**：不逆大势 + 三次熔断 + 止损必绑 + 移动锁利
- 🛡️ **持仓管理**：1.5R 保本止损 + 2R 追踪止盈（步长2%）
- 📈 **回测引擎**：MGBX 全量历史 K 线，严格 MGBX 盈亏公式 + 0.05% Taker 手续费
- 🌐 **多币种验证**：ETH 最佳（波动率适中），BTC 稳健，SOL 可用但不推荐
- 📄 **文档美化**：中英双语 `<details>` 折叠，完整逐笔明细
...
- 🎯 **策略定型**：确认 v1 原版为最终版本，全仓60%保证金驱动的双周期滚仓引擎
- 📊 **四币全量回测**：ETH +1,122% / BTC +696% / SOL +261% / LAB 淘汰
- 🔬 **版本演进**：v1-v6 六版对比，v1 在所有优化尝试中胜出
- 📋 **风控铁律**：不逆大势 + 三次熔断 + 止损必绑 + 移动锁利
- 🛡️ **持仓管理**：1.5R 保本止损 + 2R 追踪止盈（步长2%），80%+ 盈利触发完整保护链
- 📈 **回测引擎**：MGBX 全量历史 K 线，严格 MGBX 盈亏公式 + 0.05% Taker 手续费
- 🌐 **多币种验证**：ETH 最佳（波动率适中），BTC 稳健，SOL 可用但不推荐
- 📄 **文档美化**：中英双语 `<details>` 折叠切换，完整逐笔明细见 `BACKTEST_MAX.md`
